        ...
        adapter
            .as_hal::<Vulkan, _, _>(|hal_adapter| {
                ...
                let mut enabled_extensions =
                    hal_adapter.required_device_extensions(required_features);
                for extension in [
                ...
                for extension in [
                    ...
                    if !found {
                        return Err(format!("wgpu: Vulkan device extension {wanted} is not available"));
                    }
                }
...
fn device_context() -> Result<&'static EngineDeviceContext, String> {
    DEVICE_CONTEXT.get_or_init(|| {
        ...
        let (device, queue) = if cfg!(target_os = "linux") && info.backend == wgpu::Backend::Vulkan
        {
            request_linux_vulkan_device_with_dmabuf(adapter.as_ref(), required_features, &limits)?
        } else {
...
impl Renderer {
    ...
    fn recreate_depth(&mut self) {
        let (texture, view) =
            create_depth_texture(self.ctx.device.as_ref(), self.width.max(1), self.height.max(1));
    ...
    fn create_dxgi_surface(&mut self, width: u32, height: u32) -> Result<usize, String> {
        self.resize(width, height);
        let mut target = present::create_dxgi_shared_present_target(self.ctx.device.as_ref(), width, height)?;
        let handle = target
            .take_dxgi_handle()
    ...
    fn render(&mut self) -> Result<bool, String> {
        ...
        let now = Instant::now();
        let dt = now.saturating_duration_since(self.last_frame_at).as_secs_f32();
...
pub(crate) fn renderer_backend_info(handle: u64) -> Result<BackendInfo, String> {
    let renderer = lookup_renderer(handle).ok_or_else(|| "renderer handle not found".to_string())?;
...
pub(crate) fn mtl_device_ptr_for_handle(handle: u64) -> *mut c_void {
    lookup_renderer(handle)
        .and_then(|renderer| renderer.lock().ok().map(|renderer| renderer.ctx.mtl_device_ptr as *mut c_void))
